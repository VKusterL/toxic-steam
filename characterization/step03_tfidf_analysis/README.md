...
memory at once (both toxic and non-toxic, since the whole point is
comparing the two groups).

## What it does, in order

1. Reads and concatenates every file from step02's
   flat output directory - **every** review, toxic and non-toxic
...
3. Cleans review text for TF-IDF specifically (lowercase, strip URLs/
   ...
   scoring, since this feeds a bag-of-words vectorizer, not a transformer
   model.
...
1. Copy this whole folder (`step03_tfidf_analysis/`) over - it's
...
2. Copy step02's output over too - specifically
   `steam-data/step02-output/` (flat, `review_lang` is a column). Keep
   `steam-data/` next to `characterization/` (same parent folder) - the
   command below uses `../../steam-data/...`, relative to this folder.
...
python run_tfidf.py \
  ...
  --output-dir ../../steam-data/step03-output
```

- `--input` is step02's output directory (the one containing the
  scored `*.parquet` files, `review_lang` as a column) - not step01's output.
...
## Top terms per class (the paper's TF-IDF table)

`run_top_terms.py` re-sorts the lexicon into the two ranked lists the paper
reports side by side as "Regular Trend" and "Toxic Trend". Nothing is
recomputed - every term already carries both a `tfidf_toxico` and a
`tfidf_neutro`, so the two lists are two sorts of the same file.

```bash
python run_top_terms.py \
  --input ../../steam-data/step03-output/tfidf_lexicon_en.csv \
  --output ../../steam-data/step03-output/top_terms_en.csv
```

The defaults reproduce the published table: `--top-n 15` and `--by tfidf`
(each class ranked by its own mean TF-IDF weight). `--by diferenca` or
`--by log_ratio` rank by how far the classes diverge on a term instead -
useful for a different question, but not what the paper's table shows.

The discriminative ratios quoted in the text (`penis` at about x976,
`ass` at x110.7, `kill` at x17.4) are the lexicon's own
`proporcao_toxico_neutro` column, read directly from
`tfidf_lexicon_en.csv`.

## Language

The paper reports English only, so `--lang en` is what reproduces its
numbers. The scripts remain language-parameterized and still accept `pt`;
nothing downstream in this folder depends on which languages were run.
