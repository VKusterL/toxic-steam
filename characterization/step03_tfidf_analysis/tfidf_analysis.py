...
detoxify_score >= 0.9 - see step02's toxicity_mask.py) and the same
invalid-score exclusion (rows where either
...
This reads EVERY review in step02's output, toxic and non-toxic alike -
the whole point of this analysis is comparing term weights BETWEEN the
two groups.
"""
import re
import unicodedata
...
# term extraction - the same patterns stripped before Detoxify scoring
# (see step02's detoxify_scoring.py) - so these phrases don't show up as literal vocabulary terms and skew the
...
def load_reviews_for_language(step02_dir: Path, lang: str) -> pd.DataFrame:
    ...
    language - both toxic and non-toxic rows. step02's output holds every scored language together, not one
    ...
    selects this language's rows out of the shared file (same as
    review_examples/show_review_examples.py)."""
...
def clean_text_for_tfidf(series: pd.Series) -> pd.Series:
    ...
    scoring, but the same boilerplate patterns either way."""
