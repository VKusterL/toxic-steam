...
Cleans the raw games/users/reviews files, deduplicates reviews, and
assigns each review's language purely from `langdetect`.

This is what produces the paper's working corpus: 36,823,127 English
reviews from 14,183,630 unique users across 67,477 titles with at least
one English review.

Two of these four steps are heavy (review cleaning+dedup, language
detection) and are meant to run on a many-core machine, not a laptop -
they're marked **HEAVY** below. Games/users cleaning is light and can run
anywhere.
...
(`characterization/step01_cleaning_and_language_detection/`), go up twice to
leave `characterization/` and into its sibling `steam-data/`. This works
unchanged on any machine as long as `characterization/` and `steam-data/`
are copied/mounted next to each other (same parent folder) - no path needs
editing per machine.
...
tables, not the full text corpus duplicated; step 3's
...
## What step 02 consumes

`step02_run_detoxify` reads step 4's `reviews_cleaned.parquet/` and keeps
only rows where `review_lang == perspective_declared_language` (both
signals agreeing on the same language). That filter is applied inline in
`detoxify_scoring.apply_language_mask`; nothing here needs to write a
second, pre-filtered copy of the corpus.
