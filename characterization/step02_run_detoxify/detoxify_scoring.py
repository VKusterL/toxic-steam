...
(`perspective_declared_language == review_lang`) - applied inline in
apply_language_mask below.
...
output file per source file. Downstream steps (step03, review_examples)
filter `review_lang == lang` themselves after reading,
...
Unlike toxicity_mask.py/validity_mask.py's slim report-only summaries, the output here keeps every column already in step01's
